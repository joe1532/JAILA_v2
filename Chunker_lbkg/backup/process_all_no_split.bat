...
REM --- Chunker Batch Script (UDEN SPLIT) ---
REM Kører chunkerlbkg.py på alle .md-filer i input-mappen UDEN at splitte chunks
REM 
REM SPLIT-FUNKTIONALITET: DEAKTIVERET (--max-tokens 0)
REM For at aktivere split: brug process_all.bat i stedet
REM
REM --- Konfiguration ---
...
echo.
echo Starter behandling af .md-filer fra '%INPUT_DIR%' (UDEN SPLIT)...
echo.
...
for %%f in ("%INPUT_DIR%\*.md") do (
    ...
    REM Kør Python-scriptet UDEN split-funktionalitet
    python "%SCRIPT_PATH%" --input "%%f" --out-prefix "!OUTPUT_PREFIX!" --no-csv --max-tokens 0
...
echo --------------------------------------------------
echo.
echo Alle filer er blevet behandlet (UDEN SPLIT).
