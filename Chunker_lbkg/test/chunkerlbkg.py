...
def redistribute_metadata(original_chunk: Dict[str, Any], text_parts: List[str]) -> List[Dict[str, Any]]:
    ...
    for i, text_part in enumerate(text_parts):
        ...
        # Opdater atom_id med part-info (atom_base_id forbliver uændret)
        original_atom_id = part_chunk.get("atom_id", "")
        if original_atom_id:
            # Indsæt part-info før kind
            ...
            # atom_base_id forbliver uændret for grouping/routing
...
def make_atom_base_id(law_id: str, paragraf: Optional[str] = None, stk: Optional[str] = None, 
                      nr: Optional[str] = None, lit: Optional[str] = None, pkt: Optional[str] = None,
                      kind: str = "rule", note_id: Optional[str] = None, section: Optional[str] = None) -> str:
    """Generér base atom_id uden part-suffix for grouping/routing."""
    return make_atom_id(law_id, paragraf, stk, nr, lit, pkt, None, kind, note_id, section)
...
def make_atom_id(law_id: str, paragraf: Optional[str] = None, stk: Optional[str] = None, 
    ...
    law_id--parpar--stkstk--nrnr--litlit--pktpkt--parti-of-n--kindkind--noteN
    Bruger 'par' i stedet for '§' for ASCII-sikkerhed.
    """
    parts = [law_id]
    
    # Section-baserede chunks (afsnit/kapitel)
    if section and not paragraf:
        # For section chunks: law_id--section<normalized>--kindsection
    ...
    if paragraf:
        parts.append(f"par{normalize_coordinate(paragraf)}")  # ASCII-sikker: par i stedet for §
    ...
    # Kind (atomtype) - KRITISK: Medtag altid for at undgå kollisioner
    parts.append(f"kind{kind}")
...
def parse_document(lines: List[str]) -> Tuple[List[Dict[str, Any]], Dict[str, str], Dict[str, str]]:
    ...
    def flush_chunk() -> None:
        ...
        # Generér atom_id og atom_base_id
        ...
        chunks.append({
            ...
            "atom_id": atom_id,  # Deterministisk ID med part-info
            "atom_base_id": atom_base_id,  # Base ID uden part-suffix
...
def update_note_atom_ids_with_context(chunks: List[Dict[str, Any]]) -> None:
    """Opdater note atom_ids til at inkludere koordinat-kontekst når tilgængelig."""
    for chunk in chunks:
        if chunk.get("level") == "note":
            # Tjek om noten har koordinat-kontekst
            paragraf = chunk.get("paragraf")
            stk = chunk.get("stk") 
            nr = chunk.get("nr")
            note_id = str(chunk.get("note_id", ""))
            
            if paragraf:  # Kun hvis vi har koordinat-kontekst
                # Generer ny atom_id og atom_base_id med koordinat-kontekst
                law_id = "KSL_2025-04-11_nr460"
                new_atom_base_id = make_atom_base_id(
                    law_id=law_id,
                    paragraf=paragraf,
                    stk=stk,
                    nr=nr,
                ...
                # Opdater begge ID'er
                old_atom_id = chunk.get("atom_id", "")
                old_atom_base_id = chunk.get("atom_base_id", "")
                chunk["atom_id"] = new_atom_id
                chunk["atom_base_id"] = new_atom_base_id
                
                # Debug info
                if old_atom_id != new_atom_id:
                    print(f"🔄 Note atom_id opdateret: {old_atom_id} → {new_atom_id}")
                if old_atom_base_id != new_atom_base_id:
                    print(f"🔄 Note atom_base_id opdateret: {old_atom_base_id} → {new_atom_base_id}")


def validate_atom_id_uniqueness(chunks: List[Dict[str, Any]]) -> None:
    """Validér at alle atom_id'er er unikke. Fail hard ved dubletter."""
    atom_id_count = {}
    duplicates = []
    
    for chunk in chunks:
        atom_id = chunk.get("atom_id")
        if atom_id:
            if atom_id in atom_id_count:
                atom_id_count[atom_id] += 1
                if atom_id_count[atom_id] == 2:  # Første duplikat
    ...
    if duplicates:
        ...
        for dup_id in duplicates:
            print(f"   🔴 Duplikat: {dup_id} (forekommer {atom_id_count[dup_id]} gange)")
            # Find chunks med dette ID for debugging
            matching_chunks = [c for c in chunks if c.get("atom_id") == dup_id]
            for i, chunk in enumerate(matching_chunks):
                print(f"      [{i+1}] {chunk.get('chunk_id')} (level: {chunk.get('level')}, kind: {chunk.get('kind')})")
...
def main():
    ...
    # Update note atom_ids with coordinate context (after context is applied)
    update_note_atom_ids_with_context(chunks)
    ...
    # Validér atom_id unikhed (før split)
    ...
    if args.max_tokens and args.max_tokens > 0:
        ...
        # Validér atom_id unikhed igen efter split
        validate_atom_id_uniqueness(chunks)
